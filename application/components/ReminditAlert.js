...
class ReminditAlert extends React.Component {
    ...
    render() {
        return (
            <Animated.View style={{ ...this.styles.alert, height: this.animatedValue }}>
                {this.state.dropAlert && <Lottie source={successanimation} autoPlay loop={false} resizeMode="contain" style={{ width: 80, height: 80 }} />}
                <View>
    ...
    styles = StyleSheet.create({
        alert: {
            ...
            backgroundColor: this.state.dropAlert ? this.state.dropAlert.color : '#2ecc71',
            zIndex: 10,
        },
        text: {
            fontSize: 16,
